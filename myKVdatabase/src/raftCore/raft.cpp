...
void Raft::RequestVote(const raftRpcProctoc::RequestVoteArgs* args, raftRpcProctoc::RequestVoteReply* reply) {
  ...
  int lastLogTerm = getLastLogTerm();// 当前节点日志中最后一条日志的 term
  ...
  // 候选人在 同一个 term 内 进行 重复投票请求（retry）选举过程不一定一次成功,所以，为了提升选举成功概率，candidate 会重发（retry）RequestVote 请求。
  //     当因为网络质量不好导致的请求丢失重发就有可能！！！！处理网络丢包或延迟，确保幂等性
  // m_votedFor != -1初始值通常是 -1，表示当前 term 尚未投票；
